...
def test_deposit(super, token, accounts):
...
def test_withdraw_with_reward(
    ...
    stable.initialize(tokenW, tokenK, 4, 4, 0, 1e18, {'from': accounts[0]})
    ...
    tokenK.approve(new_station_addr4, 10000e18, {'from': accounts[0]})
    ...
    while k < 10:
        ...
        tokenK.approve(new_station_addr4, 3000e18, {'from': accounts[0]})
        for i in range(swap_count):
            station4.swap_tokens(100e18, 99e18, tokenW, 1e18, {'from': accounts[0]})
        for i in range(swap_count):
            station4.swap_tokens(100e18, 99e18, tokenK, 1e18, {'from': accounts[0]})
        k += 1
    k = 0
...
def test_update_owner_event_fires(super, accounts):
